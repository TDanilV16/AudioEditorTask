...
class CutAudioOperation(SetAudioEditorOperation):
    def __init__(self, input_file_name, start, stop, tmp_dir,
                 delete_old="f"):
    ...
    def __call__(self):
        self.exit_file_name = get_exit_file_name("cut",
                                                 self.input_file_name)
        if self.delete_old == "t":
            command = "ffmpeg -i {} -ss {} -to {} {}".format(
                self.input_file_name, self.start, self.stop,
                self.exit_file_name)
            ...
            shutil.copy(self.input_file_name,
                        self.tmp_dir + "/" + self.input_file_name)
            ...
            process.communicate()
            chronicler.write_history_of_file(self.exit_file_name,
                                             "обрезаная копия исходного")
        else:
            return f"Неправильно указан параметр модификации " \
                   f"исходного аудио. Ожидали: f/t, получили: " \
                   f"{self.delete_old}"

    def undo(self):
        if self.delete_old == "f":
            shutil.copy(self.exit_file_name,
                        self.tmp_dir + "/" + self.exit_file_name)
            ...
            shutil.copy(self.exit_file_name,
                        self.tmp_dir + "/" + self.exit_file_name)
            chronicler.write_history_of_file(self.exit_file_name, "удален")
            chronicler.write_history_of_file(self.input_file_name,
                                             "восстановлен")
            os.remove(self.exit_file_name)
            shutil.move(self.tmp_dir + "/" + self.input_file_name,
                        os.getcwd())


class ConcatAudioOperation(SetAudioEditorOperation):
    def __init__(self, exit_file_name, audio_files, tmp_dir,
                 delete_old="f"):
    ...
    def __call__(self):
        if self.delete_old == "f":
            ...
            command = "ffmpeg -i concat:{} {}".format(str(t),
                                                      self.exit_file_name)
            process = subprocess.Popen(command)
            process.communicate()
            chronicler.write_history_of_file(self.exit_file_name,
...
class SpeedUpAudioOperation(SetAudioEditorOperation):
    ...
    def __call__(self):
        if 1 <= float(self.times) <= 100:
            self.exit_file_name = get_exit_file_name("speedup",
                                                     self.input_file_name)
            if self.delete_old == "f":
                command = "ffmpeg -i {} -af atempo={} {}".format(
                    self.input_file_name, self.times, self.exit_file_name)
                process = subprocess.Popen(command)
                process.communicate()
                chronicler.write_history_of_file(self.exit_file_name,
                                                 "ускоренная копия "
            ...
            return "Параметр изменения звука должен быть в пределах " \
                   "от 1 до 100. Попробуйте еще раз."
    ...
    def undo(self):
        if self.delete_old == "f":
            ...
            shutil.copy(self.tmp_dir + "/" + self.input_file_name,
                        os.getcwd())
            chronicler.write_history_of_file(self.input_file_name,
                                             "восстановлен")
            shutil.copy(self.exit_file_name,
...
class SlowDownAudioOperation(SetAudioEditorOperation):
    ...
    def __call__(self):
        if 0.5 <= float(self.times) <= 1:
            self.exit_file_name = get_exit_file_name("slowdown",
                                                     self.input_file_name)
            if self.delete_old == "f":
                command = "ffmpeg -i {} -af atempo={} {}".format(
                    self.input_file_name, self.times,
                    self.exit_file_name)
                process = subprocess.Popen(command)
                process.communicate()
                chronicler.write_history_of_file(self.exit_file_name,
...
class ChangeAudioVolumeOperation(SetAudioEditorOperation):
    ...
    def __call__(self):
        self.exit_file_name = get_exit_file_name("volume",
                                                 self.input_file_name,
                                                 self.action)
        if self.action == "i":
            if "dB" in self.k:
                command = 'ffmpeg -i {} -filter:a "volume={}" {}'.format(
                    self.input_file_name, self.k,
                    self.exit_file_name)
            else:
                command = 'ffmpeg -i {} -filter:a "volume={}" {}'.format(
                    self.input_file_name, 1 + float(self.k),
                    self.exit_file_name)
        elif self.action == "r":
            if "dB" in self.k:
                command = 'ffmpeg -i {} -filter:a "volume=-{}" {}'.format(
...
def get_exit_file_name(action, input_file_name, volume_action=None):
    ...
    return "{}_{}_{}{}".format(volume_action, action, file_name,
                               file_format)
